...
const CheckoutPage = () => {
  ...
  useEffect(() => {
    if (status === "authenticated" && data) {
      const getcartitems = async () => {
        const getaddres = await fetch("api/address");
        const addresponse = await getaddres.json();
        if (getaddres.ok) {
          setSavedAddresses(addresponse);
        }
        const response = await fetch(`/api/cart`);
        const cartData = await response.json();
        const cleanedItems = cartData.map(
          ({ userId, stockCount, ...item }) => item,
        );
  ...
  const handleSaveAddress = async () => {
    try {
      const save_address = await fetch("/api/address", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(shippingForm),
      });
      const addresponse = await save_address.json();
      if (save_address.ok) {
  ...
  const handlePlaceOrder = async () => {
    try {
      const isEmptyField = Object.entries(shippingForm).some(
    ...
    <div className="min-h-screen text-black bg-gray-50 flex flex-col">
      ...
      <main className="grow container mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 lg:py-8">
        <div className="mb-6 flex items-center justify-between">
          <button
            onClick={handleback}
            className="text-gray-600 flex items-center hover:text-gray-900 active:text-gray-950 transition-colors"
          >
            <svg
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-5 lg:p-6">
              ...
              {savedAddresses.length > 0 && (
                <div className="mb-5 sm:mb-6">
                  <p className="text-xs sm:text-sm font-medium text-gray-600 mb-2.5 flex items-center gap-1.5">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                    {savedAddresses.map((addr) => (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-400 truncate mt-0.5">{addr.phone}</p>
                      </div>
                    ))}
                    <div
                      onClick={() => {
                        ...
                        setShippingForm({
                          ...
                          address: "", apartment: "", city: "", zipCode: "",
                        });
                      }}
                      className={`border-2 border-dashed rounded-lg p-3 cursor-pointer transition-all duration-200 flex flex-col items-center justify-center gap-1 min-h-[80px] ${
                    ...
                    >
                      ...
                      <span className="text-xs font-medium text-gray-500">New Address</span>
                    </div>
                  </div>
                </div>
              )}
                ...
                {[
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-sm sm:text-base font-semibold ${method.priceValue === 0 ? "text-green-600" : "text-gray-900"}`}
                      >
                        {method.price}
                      </span>
                    </div>
              ...
              {paymentMethod === "cod" && (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 sm:p-4 mt-3 sm:mt-4">
                  <div className="flex items-start gap-2">
                    <svg
                      className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-600 shrink-0 mt-0.5"
                      fill="none"
                      stroke="currentColor"
                      viewBox="0 0 24 24"
                    >
                    ...
                    <div>
                      ...
                      <p className="text-[10px] sm:text-xs text-yellow-700">
                        Have exact cash ready upon delivery
                      </p>
                    </div>
                  </div>
                </div>
                    ...
                    >
                      <div className="w-12 h-12 sm:w-14 sm:h-14 bg-gray-100 rounded-lg flex items-center justify-center shrink-0">
                        <Image
                          src={
                            item.image ||
                            "/uploads/products/" + productSlug + ".png"
                          }
                          alt={item.name}
                          width={50}
                          height={50}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 sm:p-8 animate-[fadeIn_0.2s_ease-out]">
            <div className="flex flex-col items-center text-center">
              ...
              <h3 className="text-lg font-bold text-gray-900 mb-1">Save this address?</h3>
              ...
              <div className="flex gap-3 w-full">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-2.5 bg-gray-900 text-white rounded-xl font-semibold text-sm hover:bg-gray-800 active:bg-gray-950 transition-colors"
                >
                  Save Address
                </button>
              </div>
